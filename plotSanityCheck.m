...
%             transformedPoints(1,:) = (data_split_with_ring{i}(ring).points(1,:)+ 0.005).*sin(data_split_with_ring{i}(ring).points(2,:)).*cos(data_split_with_ring{i}(ring).points(3,:));
%             transformedPoints(2,:) = (data_split_with_ring{i}(ring).points(1,:)+ 0.005).*sin(data_split_with_ring{i}(ring).points(2,:)).*sin(data_split_with_ring{i}(ring).points(3,:));
%             transformedPoints(3,:) = (data_split_with_ring{i}(ring).points(1,:)+ 0.005).*cos(data_split_with_ring{i}(ring).points(2,:));
            ...
            scatter3(transformedPoints(1,:),transformedPoints(2,:),transformedPoints(3,:), 50, '.');
        end
    end
