function [delta, opt]= estimateDeltaFromMechanicalModel(opt, data, plane, delta, num_beams, num_targets)
    tic;
...
%         T = optimvar('T', 1, 3, 'LowerBound',[-0.05 -0.05 -0.05],'UpperBound',[0.05 0.05 0.05]); % 1x3
        ...
        prob.Objective = f;
        x0.D_corr = opt_tmp.D_corr_init;
        x0.theta_corr = opt_tmp.theta_corr_init;
        x0.phi_corr = opt_tmp.phi_corr_init;
%         x0.T = opt.T_init;
...
%         delta(ring).H(1:3, 4) = sol.T;
        ...
        opt_tmp.computation_time(ring).time = toc;
    end
    opt = opt_tmp;
end