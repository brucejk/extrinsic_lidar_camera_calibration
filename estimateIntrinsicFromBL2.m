function  [delta, plane, valid_targets] = estimateIntrinsicFromBL2(num_beams, num_targets, num_scans, data_split_with_ring, data_split_with_ring_cartesian, object_list)
    delta(num_beams).D_s = struct();
    delta(num_beams).D = struct();
    delta(num_beams).A_c = struct();
    delta(num_beams).S_vc = struct();
    delta(num_beams).C_vc = struct();
    delta(num_beams).H_oc = struct();
    delta(num_beams).S_voc = struct();
    delta(num_beams).C_voc = struct();
%     delta(num_beams).V_c = struct(); Mentioned but not used in their formulation
%     delta(num_beams).V_oc = struct();
    ...
    for i = 1: num_scans
        ...
        [delta, ~, valid_targets] = estimateDeltaFromBL2(opt.delta, data_split_with_ring, plane, delta(num_beams), num_beams, num_targets);
    end
end
